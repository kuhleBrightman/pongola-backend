...
app.post('/api/products/:productId/images', upload2.array('images', 5), (req, res) => { // 'images' is the field name, 5 is max count
    ...
    // Check if the product exists before adding images (optional but good practice)
    ...
    db.query(checkProductSql, [productId], (err, productResults) => {
        if (err) {
            ...
            // Clean up uploaded files if product check fails
            files.forEach(file => fs.unlink(file.path, (unlinkErr) => { if (unlinkErr) console.error('Error deleting file:', unlinkErr); }));
            return res.status(500).json({ message: 'An error occurred while checking product existence.' });
        }
        ...
        // Check the current number of images for this product
        const countImagesSql = 'SELECT COUNT(*) AS imageCount FROM `product_image` WHERE `ProductID` = ?;'; // Use product_image and ProductID
        db.query(countImagesSql, [productId], (err, countResults) => {
            if (err) {
                console.error('Error counting existing images:', err);
                // Clean up uploaded files
                files.forEach(file => fs.unlink(file.path, (unlinkErr) => { if (unlinkErr) console.error('Error deleting file:', unlinkErr); }));
                return res.status(500).json({ message: 'An error occurred while counting existing images.' });
            }

            const existingImageCount = countResults[0].imageCount;
            const totalImagesAfterUpload = existingImageCount + files.length;

            if (totalImagesAfterUpload > 5) { // Enforce the 5 image limit
                // Clean up uploaded files
                files.forEach(file => fs.unlink(file.path, (unlinkErr) => { if (unlinkErr) console.error('Error deleting file:', unlinkErr); }));
                return res.status(400).json({ message: `Product can have a maximum of 5 images. You already have ${existingImageCount} and tried to upload ${files.length}.` });
            }

            // Get the maximum SortOrder for existing images to determine the next SortOrder
            ...
            db.query(getMaxSortOrderSql, [productId], (err, maxSortOrderResults) => {
                if (err) {
                    console.error('Error getting max SortOrder:', err);
                    // Clean up uploaded files
                    files.forEach(file => fs.unlink(file.path, (unlinkErr) => { if (unlinkErr) console.error('Error deleting file:', unlinkErr); }));
                    return res.status(500).json({ message: 'An error occurred while determining SortOrder.' });
                }

                const maxSortOrder = maxSortOrderResults[0].maxSortOrder || -1; // Use -1 if no existing images
                let currentSortOrder = maxSortOrder + 1;

                // Prepare data for inserting into the product_image table
                const imageValues = files.map((file, index) => [
                    productId, // Use ProductID
                    file.filename, // Store just the filename in Image_url
                    // Assign SortOrder: 0 for the first image if none exist, otherwise increment
                    existingImageCount === 0 && index === 0 ? 0 : currentSortOrder++
                ]);

                // SQL query to insert multiple image records into product_image
                const insertImagesSql = `
                    INSERT INTO \`product_image\` (\`ProductID\`, \`Image_url\`, \`SortOrder\`)
                    VALUES ?; -- Use ? for inserting multiple rows
                `;
                ...
                db.query(insertImagesSql, [imageValues], (err, result) => {
                    if (err) {
                        ...
                        files.forEach(file => fs.unlink(file.path, (unlinkErr) => { if (unlinkErr) console.error('Error deleting file:', unlinkErr); }));
                        return res.status(500).json({ message: 'An error occurred while saving image information.' });
                    }

                    // Return success response with info about inserted rows
                    res.status(201).json({
                        message: 'Images uploaded and records created successfully!',
                        uploadedCount: files.length,
                        firstInsertId: result.insertId // ID of the first inserted row
                    });
                });
            });
        });
